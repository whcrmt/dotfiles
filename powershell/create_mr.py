...
def create_mr(user_id, private_token, gitlab_url, http_remote, ssh_remote, title):
    ...
    if project_url.startswith(http_remote):
        ...
        print(f"Project did not start with http:// or git@, set remote URL with: git remote set-url origin <http url>")
        logging.error(f"Project did not start with http:// or git@, set remote URL with: git remote set-url origin <http url>")
        return

    if project_join.endswith(".git"):
        project_path = project_join[:-4]
    else:
        print(f"Project did not end in .git, set remote URL with: git remote set-url origin <http url>")
        logging.error(f"Project did not end in .git, set remote URL with: git remote set-url origin <http url>")
        return
